      ...
      link: function (scope: MediaLibraryScope, element, attributes) {
        ...
        const HEADER_BROWSE: Header = {
          i18Key: `${
            $(window).width() <= ui.breakpoints.tablette
              ? "library.header.browse.mobile"
              : "library.header.browse"
          }`,
        ...
        scope.showHeader = function (tab: Header) {
          if (scope.delegate && scope.delegate.handleShow) {
            const hasHandled = scope.delegate.handleShow(scope, {
              cancelAll,
              displayHighLights,
              showTemplate,
            });
        ...
        scope.selectDocuments = async () => {
          ...
          if (
            ...
            (scope.ngModel &&
              scope.multiple &&
              (scope.ngModel as Document[]).length)
          ) {
            $timeout(() => {
              scope.ngChange();
            });
        ...
        scope.importFiles = function (files) {
          ...
          for (var i = 0; i < files.length; i++) {
            ...
            const visibility =
              scope.visibility == "external" ? "protected" : scope.visibility;
