...
export let embedder = ng.directive("embedder", [
  ...
  "$filter",
  "VideoUploadService",
  function ($timeout, $filter, VideoUploadService: VideoUploadService) {
    return {
      restrict: "E",
      scope: {
        delegate: "=",
      ...
      link: function (scope: VideoScope, element, attributes, lnkNgModel) {
        ...
        Me.hasWorkflowRight("video.upload").then(hasIt => {
          hasVideoUpload = hasIt;
        }); // Make the visible() property reactive.
        ...
        scope.headers = function () {
          ...
          return scope.delegate && scope.delegate.augmentHeaders
            ? scope.delegate.augmentHeaders(headers)
            : headers;
        };
        ...
        if (!(window as any).toBlobPolyfillLoaded) {
          http()
            .get("/infra/public/js/toBlob-polyfill.js")
            .done(response => {
              eval(response.data);
        ...
        const previousImage = () => {
          const start =
            scope.upload.documents.indexOf(scope.display.editedDocument) - 1;
        ...
        scope.previousImage = () =>
          (scope.display.editedDocument = previousImage());
        ...
        scope.showHeader = function (tab: Header) {
          ...
          if (scope.delegate && scope.delegate.handleShow) {
            ...
            const hasHandled = scope.delegate.handleShow(scope as any, {
              cancelAll,
              displayHighLights,
              showTemplate,
            });
        ...
        scope.orderByField = function (field: string): void {
          ...
          else if (scope.isOrderedDesc(field)) return scope.orderByDefault();
          else scope.orderFieldDocument = field;
        ...
        const refresh = function () {
          scope.documents = filteredDocuments(
            MediaLibrary[scope.display.listFrom]
          );
          ...
          if (scope.display.search && scope.display.search.trim().length > 0) {
            return (
              idiom
                ...
                .indexOf(
                  idiom.removeAccents(scope.display.search.toLowerCase())
                ) !== -1
            );
          } else {
            return true;
        ...
        function logVideoEvent_AddToContent(document: Document) {
          if (typeof document.metadata.captation !== "undefined") {
            ...
            http()
              .postJson("/video/event/embed", videoEventData)
              .done(function (res) {
                console.log(res);
              });
          }
        ...
        scope.selectedDocuments = () =>
          scope.documents ? scope.documents.filter(d => d.selected) : [];
        ...
        scope.selectDocuments = async () => {
          ...
          if (
            scope.visibility === "external" ||
            (scope.openedFolder.filter == "protected" &&
              scope.visibility === "protected") ||
            (scope.openedFolder.filter == "public" &&
              scope.visibility === "public")
          ) {
            if (scope.multiple) {
              scope.ngModel =
                embedderService.getHtmlForVideoStreams(selectedDocuments);
            } else {
              scope.ngModel = embedderService.getHtmlForVideoStream(
                selectedDocuments[0]
              );
            }
          } else {
            ...
            if (scope.multiple) {
              scope.ngModel =
                embedderService.getHtmlForVideoStreams(duplicateDocuments);
            } else {
              scope.ngModel = embedderService.getHtmlForVideoStream(
          ...
          if (scope.ngModel) {
            setTimeout(() => {
              ...
              selectedDocuments.forEach(selectedDocument =>
                logVideoEvent_AddToContent(selectedDocument)
              );
            });
          }
        };
        ...
        scope.confirmImport = async () => {
          ...
          if (scope.visibility == "public") {
            ...
            const lastIndex =
              MediaLibrary["publicDocuments"].documents.all.length - 1;
            if (lastIndex > -1) {
              MediaLibrary["publicDocuments"].documents.all[
                lastIndex
              ].selected = true;
            }
        ...
        scope.importFiles = function (files) {
          ...
          for (var i = 0; i < files.length; i++) {
            ...
            const fileExt = files[i].name.substring(
              fileExtPos < 0 ? 0 : fileExtPos + 1
            );
              ...
              .then(result => {
                if (result.data) {
                  if (result.data.state === "error") {
                    ...
                    scope.uploadGuard.guardObjectReset();
                    notify.error(
                      result.data.code ? result.data.code : "video.file.error"
                    );
                  } else {
                    ...
                    http()
                      .postJson("/video/event/save", videoEventData)
                      .done(function (res) {
                        console.log(res);
                      });
                  }
        ...
        scope.unselectProvider = function () {
          let preview = element.find(
            "." + scope.display.provider.name + " .preview"
          );
          preview.html("");
