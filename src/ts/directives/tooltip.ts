...
type TooltipAttributes = {
  ...
  tooltipRestrictSelector: string;
  tooltipCheck: string;
};
abstract class AbstractToolTip {
  protected tip: JQuery<HTMLElement>;
  ...
  constructor(
    ...
    if (attributes.tooltipRestrictSelector && this.targetElement) {
      this.restrictToElement = this.targetElement.parents(
        attributes.tooltipRestrictSelector
      );
    ...
    if (element[0].hasAttribute("tooltip-check")) {
      this.scope.$watch(
        () => scope.$eval(attributes.tooltipCheck),
        (newVal) => (this.tooltipCheck = !!newVal)
      );
    }
    // Handle multiline
  ...
  bind(): void {
    ...
    this.subscription.add(
      this.onChange.debounceTime(400).subscribe((s) => {
        if (s == "hide") {
          this.doRemove();
        } else {
          this.doShow();
...
class CustomToolTip extends AbstractToolTip {
  protected createTip() {
    ...
    this.tip = $("<div />")
      .addClass("tooltip")
      .html(
        this.$compile("<div ng-include='\"" + templatePath + "\"'></div>")(
          this.scope
        )
      )
      .appendTo("body");
  ...
  protected moveTip() {
    ...
    if (restrictToElement) {
      if (left < restrictToElement.offset().left) {
        left = parseInt(
          targetElement.offset().left + $(targetElement).width() + 5
        );
      }
      if (
        left + $(targetElement).width() >
        restrictToElement.offset().left + $(restrictToElement).width()
      ) {
    ...
    tip.css(
      "left",
      document.body.clientWidth >= left + tip.outerWidth()
        ? left
        : document.body.clientWidth - tip.outerWidth() - 1
    );
    tip.css("top", top);
  }
}
...
class DefaultToolTip extends AbstractToolTip {
  protected isReady() {
    if (
      ...
      !this.attributes[this.tooltipType] ||
      !this.tooltipCheck ||
      this.attributes[this.tooltipType] === "undefined"
    ) {
      return false;
    }
  ...
  protected createTip() {
    this.tip = $("<div />")
      ...
      .html(
        this.$compile(
          '<div class="arrow"></div><div class="content">' +
            idiom.translate(this.attributes[this.tooltipType]) +
            "</div> "
        )(this.scope)
      )
      .appendTo("body");
  ...
  protected moveTip() {
    if (!this.tip || !this.isReady()) return;
    let top = parseInt(
      this.sourceElement.offset().top + this.sourceElement.outerHeight()
    );
    let left = parseInt(
      this.sourceElement.offset().left +
        $(this.sourceElement).width() / 2 -
        $(this.tip).width() / 2
    );
    if (top < 5) {
      top = 5;
    ...
    this.tip.offset({
      top: top,
      left:
        document.body.clientWidth >= left + this.tip.outerWidth()
          ? left
          : document.body.clientWidth - this.tip.outerWidth() - 1,
    });
  }
}

export const tooltip = ng.directive("tooltip", [
  "$compile",
  function ($compile) {
    return {
      restrict: "A",
...
export const tooltipOnEllipsis = ng.directive("tooltipOnEllipsis", [
  "$compile",
  function ($compile) {
    return {
      restrict: "A",
      link: function (scope, element, attributes) {
        AbstractToolTip.create({
          ...
          tooltipType: "tooltipOnEllipsis",
          sourceElement: element.parent("a"),
        });
      },
    };
  },
]);
export const odebtTooltip = ng.directive("odebtTooltip", [
  "$compile",
  function ($compile) {
    return {
      restrict: "A",
      link: function (scope, element, attributes) {
        if (attributes.odebtTooltip)
          jQuery(element).attr("title", attributes.odebtTooltip);
