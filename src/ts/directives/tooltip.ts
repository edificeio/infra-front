...
function tooltipLink ($compile, scope, element, attributes, tooltipShoudBeDisplayed){
        ...
        tgtElement.on('mouseenter', function() {
            ...
            if (attributes.tooltipTemplate) {
                ...
                if(tooltipShoudBeDisplayed(element)) {
                    tip.fadeIn();
...
export const tooltip = ng.directive('tooltip', ['$compile', function($compile){
    return {
        ...
        link: function(scope, element, attributes) {
            tooltipLink($compile, scope, element, attributes, () => true)
        }
    }
}]);
export const tooltipEllipzed = ng.directive('tooltipe', ['$compile', function($compile){
    return {
        restrict: 'A',
        link: function(scope, element, attributes) {

            tooltipLink($compile, scope, element, attributes, e => (e.parent('a').offsetWidth < e.parent('a').scrollWidth))
        }
    }
}]);