    ...
    function ($rootScope, $route, $routeParams) {
      ...
      $rootScope.$on(
        ...
        function ($currentRoute, $previousRoute) {
          if (
            routes[$route.current.action] instanceof Array &&
            (currentAction !== $route.current.action ||
              (currentParams !== $route.current.params &&
                !(
                  Object.getOwnPropertyNames($route.current.params).length === 0
                )))
          ) {
            currentAction = $route.current.action;
            currentParams = $route.current.params;
  ...
  .factory("httpWrapper", function () {
    return {
      wrap: function (name, fun, context) {
        ...
        if (typeof scope[name] !== "object") scope[name] = {};
        ...
        if (xhrReq && xhrReq.xhr) xhrReq.xhr.complete(completion);
        else completion();
      },
    };
  });
      ...
      controller: [
        ...
        function (scope, attributes) {
          ...
          scope.$on("$destroy", function () {
            template.unwatch(attrTemplate, fct);
          });
...
module.directive("soundSelect", function () {
  return {
    ...
    template:
      '<div><audio ng-src="[[ngModel]]" controls ng-if="ngModel" style="cursor: pointer"></audio>' +
    ...
    link: function (scope, element, attributes) {
      ...
      scope.selectedFile.visibility = scope.$parent.$eval(
      ...
      scope.selectedFile.visibility =
        scope.selectedFile.visibility.toLowerCase();
...
module.directive("mediaSelect", function () {
  return {
    ...
    template:
      '<div><input type="button" class="pick-file [[class]]" tooltip="[[mytooltip]]" />' +
    ...
    compile: function (element, attributes) {
      if (!attributes.mytooltip && attributes.tooltip) {
        console.warn(
          "tooltip attribute is deprecated on media-select tag, use mytooltip instead."
        );
...
module.directive("preview", function () {
  return {
    restrict: "E",
    template:
      '<div class="row content-line"><div class="row fixed-block height-four">' +
...
module.service("tracker", [
  ...
  "$rootScope",
  (function () {
    // Tracker should be defined in its own source file, instead of locally.
        ...
        switch (type) {
          case "matomo":
            try {
              ...
              if (params.Profile)
                _paq.push(["setCustomDimension", 1, params.Profile]);
              if (params.School)
                _paq.push(["setCustomDimension", 2, params.School]);
              if (params.Project)
                _paq.push(["setCustomDimension", 3, params.Project]);
              ...
              if (params.detailApps && window.entcore.template) {
                ...
                if (
                  angular.isArray(params.doNotTrack) &&
                  model &&
                  model.me &&
                  angular.isArray(model.me.apps)
                ) {
                  // Retrieve app from current URL.
                  for (var i = 0; i < model.me.apps.length; i++) {
                    if (
                      model.me.apps[i] &&
                      model.me.apps[i].address &&
                      model.me.apps[i].name &&
                      this.$location
                        .absUrl()
                        .indexOf(model.me.apps[i].address) !== -1 &&
                ...
                window.entcore.template.open = function (name, view) {
                  var ret = encapsulatedFunction.apply(
                    window.entcore.template,
                    arguments
                  );
                  if ("main" === name) {
                    ...
                    view = view || "unknown";
                    var url =
                      self.$location.absUrl().split("#")[0] + "/" + view;
              ...
              console.error(
                'Invalid tracker object. Should look like {"siteId": 99999, "url":"http://your.matomo.server.com/"}"',
                e
              );
            }
            break;
      ...
      switch (this.type) {
        case "matomo":
          ...
          if (typeof name === "string") {
            name = name.trim();
            if (name.length > 0) event.push(name);
          }
          if (typeof value === "number") event.push(value);
          _paq.push(event);
          break;
        default:
          break;
  ...
  function ($compile, tracker) {
    return {
      ...
      compile: function (element, attributes, transclude) {
        ...
        element
          .find("[logout]")
          .attr("href", "/auth/logout?callback=" + skin.logoutCallback);
        ...
        return function postLink(
          ...
          scope.goToMyAccount = () => {
            ...
            httpAxios.put("/userbook/preference/rgpdCookies", {
              showInfoTip: false,
            });
          };
          scope.closeBanner = () => {
        ...
        .done(function (data) {
          ...
          if (data && data.key) {
            ...
            document.body.appendChild(scriptZendesk).onload = () => {
              // Set the Zendesk Guide Widget settings language
              ...
              console.log("Module zendesk: " + JSON.stringify(data.module));
    ...
    function setZendeskLabels(dataModule, locationPathname) {
      ...
      if (
        ...
        console.log(
          "dataModule has labels: " +
            dataModule.labels.hasOwnProperty(moduleLabel)
        );
      ...
      // Check if the user has a profile and replace the profile label
      if (dataModule.profile.length > 0 && labels.includes("${profile}")) {
        let index = -1;
        let userProfile = profileTypes();
        for (let i = 0; i < dataModule.profile.length; i++) {
          if (dataModule.profile[i].includes(userProfile)) {
            index = i;
            break;
          }
        }
        if (index !== -1) {
          labels = labels.replace("${profile}", dataModule.profile[index]);
        }
      }

      // Check if the user has a theme and replace the theme label
      if (labels.includes("${theme")) {
        if (skin.themeName == "cg771d") {
...
module.directive("publicInfotip", [
  "$compile",
  "tracker",
  function ($compile, tracker) {
    return {
...
module.directive("portalStyles", function () {
  return {
    restrict: "EA",
    compile: function (element, attributes) {
      if (attributes.portalStyles != "false")
        $("html").addClass("portal-container");
      $("[logout]").attr(
        "href",
        "/auth/logout?callback=" + skin.logoutCallback
...
module.directive("defaultStyles", function () {
  return {
    restrict: "EA",
    link: function (scope, element, attributes) {
      if (attributes.defaultStyles != "false")
        $("html").addClass("portal-container");
      ui.setStyle(skin.theme);
    },
  };
  ...
  return {
    ...
    link: function ($scope, $element, $attributes) {
      ...
      $attributes.$observe("skinSrc", function () {
        if (
          $attributes.skinSrc.indexOf("http://") === -1 &&
          $attributes.skinSrc.indexOf("https://") === -1 &&
          $attributes.skinSrc.indexOf("/workspace/") === -1
        ) {
    ...
    link: function ($scope, $element, $attributes) {
      $element.bind("error", function () {
        if ($attributes.defaultSrc != undefined) {
          if (
            $attributes.defaultSrc.indexOf("http://") === -1 &&
            $attributes.defaultSrc.indexOf("https://") === -1 &&
            $attributes.defaultSrc.indexOf("/workspace/") === -1
          ) {
...
module.directive("pullDownContent", function () {
  return {
    ...
    transclude: true,
    template:
      '<div class="wrapper"><div class="arrow"></div><div class="pull-down-content" ng-transclude></div></div>',
    link: function (scope, element, attributes) {},
  };
});
...
module.directive("dropDownButton", function () {
  return {
    ...
    template:
      '<div class="drop-down-button hidden"><div ng-transclude></div></div>',
    link: function (scope, element, attributes) {
  ...
  return {
    ...
    link: function ($scope, $element, $attributes) {
      var addImage = function () {
        ...
        var loadingIllustrationPath =
          skin.basePath + "/img/icons/anim_loading_small.gif";
        $("<img>")
      ...
      if ($attributes.onlyLoadingIcon === undefined) {
        http().bind("request-ended." + $attributes.request, function (e) {
          var loadingDonePath =
            skin.basePath + "/img/icons/checkbox-checked.png";
          $element.find(".loading-icon").remove();
          $("<img>").attr("src", loadingDonePath).appendTo($element);
        });
      } else {
        http().bind("request-ended." + $attributes.request, function (e) {
          $element.find("img").remove();
  ...
  return {
    ...
    link: function ($scope, $element, $attributes) {
      $attributes.$observe("loadingPanel", function (val) {
        http().bind(
          "request-started." + $attributes.loadingPanel,
          function (e) {
            var loadingIllustrationPath =
              skin.basePath + "/img/illustrations/loading.gif";
...
module.directive("behaviour", function () {
  return {
    ...
    link: function ($scope, $element, $attributes) {
      console.error(
        'This directive is deprecated. Please use "authorize" instead.'
      );
      if (!$attributes.name) {
      ...
      $scope.$watch("resource", function (newVal) {
        var hide =
          ($scope.resource instanceof Array &&
            _.find($scope.resource, function (resource) {
              return (
                !resource.myRights ||
                resource.myRights[$attributes.name] === undefined
              );
            }) !== undefined) ||
          ($scope.resource instanceof Model &&
            (!$scope.resource.myRights ||
              $scope.resource.myRights[$attributes.name] === undefined));
      ...
      var switchHide = function () {
        ...
        var hide =
          (name &&
            ...
            _.find(resource, function (resource) {
              return (
                !resource.myRights || resource.myRights[name] === undefined
              );
            }) !== undefined) ||
  ...
  return {
    ...
    link: function (scope, element, attributes) {
      ui.extendElement.resizable(element, {
        ...
        thickness: attributes.resizeThickness
          ? parseInt(attributes.resizeThickness)
          : undefined,
        preserveRatio:
          attributes.preserveRatio && attributes.preserveRatio !== "false",
      });
    },
  };
});
...
const getDatePickerFormat = (): string => {
  ...
  return lang.translate(keyFormat) == keyFormat
    ? DEFAULT_FORMAT
    : lang.translate(keyFormat);
};
  ...
  function ($compile, $timeout) {
    return {
      ...
      link: function (scope, element, attributes, ngModel) {
        ...
        const getDelegate = () =>
          scope.datePickerDelegate as DatepickerDelegate;
        const onValidationChanged = () => {
          getDelegate() &&
            getDelegate().onValidationChange &&
            getDelegate().onValidationChange({ valid: ngModel.$valid });
        };
        scope.$watch("modelCtrl.$valid", () => onValidationChanged());
        scope.$on("$destroy", function () {
          getDelegate() &&
            getDelegate().onDestroy &&
            getDelegate().onDestroy({ ngModel });
        });
        getDelegate() &&
          getDelegate().onInit &&
          getDelegate().onInit({ ngModel });
        //
        ...
        const viewToMoment = (value: string) => {
          //parse from locale or from iso format
        ...
        http()
          .loadScript("/" + infraPrefix + "/public/js/bootstrap-datepicker.js")
          .then(function () {
            const firstDay = moment
              .weekdays()
              .indexOf(moment.weekdays(true)[0]);
            controller
              .init({
                format: format,
                weekStart: firstDay,
        ...
        const hideFunction = function (e) {
          if (
            e.originalEvent &&
            (element[0] === e.originalEvent.target ||
              $(".datepicker").find(e.originalEvent.target).length !== 0)
          ) {
            return;
          }
          controller.hide();
...
module.directive("datePickerIcon", function () {
  return {
    ...
    restrict: "E",
    template:
      '<div class="date-picker-icon"> <input type="text" class="hiddendatepickerform" style="visibility: hidden; width: 0px; height: 0px; float: inherit"/> <a ng-click="openDatepicker()"><i class="calendar"></i></a> </div>',
    link: function ($scope, $element, $attributes) {
      http()
        .loadScript("/" + infraPrefix + "/public/js/bootstrap-datepicker.js")
        .then(() => {
  ...
  function ($parse) {
    return {
        ...
        function ($scope) {
          ...
          $scope.matching = function (letter) {
            return function (element) {
              return (
                element[$scope.title][0].toUpperCase() === letter ||
                (letter === "#" &&
                  element[$scope.title][0].toLowerCase() ===
                    element[$scope.title][0].toUpperCase())
              );
            };
          };
          ...
          $scope.updateElements = function () {
            $scope.letters.forEach(function (letter) {
              $scope.matchingElements[letter] = _.filter(
      ...
      compile: function (element, attributes) {
        ...
        iteratorContent = iteratorContent.replace(
          '<div class="item"',
          '<div ng-repeat="' +
            iterator +
            ' |filter: matching(letter)" class="item"'
        );
        element.html(
          '<lightbox class="letter-picker" show="display.pickLetter" on-close="display.pickLetter = false;">' +
...
module.directive("completeClick", [
  "$parse",
  function ($parse) {
    return {
  ...
  function ($parse) {
    return {
      ...
      link: function (scope, element, attributes) {
        ...
        if (
          attributes.dragcondition !== undefined &&
          scope.$eval(attributes.dragcondition) === false
        ) {
    ...
    return {
      ...
      link: function (scope, element, attributes) {
        ...
        element.on("dragover", function (event) {
          if (
            attributes.dropcondition === undefined ||
            dropConditionFn(scope, { $originalEvent: event.originalEvent })
          ) {
        ...
        element.on("drop", function (event) {
          ...
          try {
            item = JSON.parse(
              event.originalEvent.dataTransfer.getData("application/json")
            );
          } catch (e) {
        ...
        function ($scope) {
          ...
          $scope.attachments = {
            ...
            removeAttachment: function (resource) {
              $scope.ngModel.assign(
                $scope,
                _.reject($scope.ngModel($scope), function (item) {
      ...
      link: function (scope, element, attributes) {
        ...
        http()
          ...
          .done(function (apps) {
            ...
            scope.attachments.display.search.application =
              scope.attachments.apps[0];
            scope.attachments.loadApplicationResources(function () {});
        ...
        scope.attachments.loadApplicationResources = function (cb) {
          ...
          var split =
            scope.attachments.display.search.application.address.split("/");
        ...
        scope.attachments.searchApplication = function () {
          ...
          Behaviours.loadBehaviours(scope.prefix, function (appBehaviour) {
            scope.attachments.resources = _.filter(
              appBehaviour.resources,
              function (resource) {
                return (
                  scope.attachments.display.search.text !== "" &&
                  (lang
                    .removeAccents(resource.title.toLowerCase())
                    .indexOf(
                      lang
                        .removeAccents(scope.attachments.display.search.text)
                        .toLowerCase()
                    ) !== -1 ||
      ...
      element.children(".opener").on("click", function (e) {
        ...
        setTimeout(function () {
          $("body").on("click.switch-side-panel", function (e) {
            if (
              !element.children(".toggle").find(e.originalEvent.target).length
            ) {
              element.addClass("hidden");
...
module.directive("stickToTop", function () {
  return {
    ...
    link: function (scope, element, attributes) {
      if (
        "noStickMobile" in attributes &&
        ui.breakpoints.checkMaxWidth("wideScreen")
      )
        return;
      ...
      $(window).scroll(function () {
        if (!initialPosition) initialPosition = element.offset().top;
...
module.directive("floatingNavigation", function () {
  return {
    ...
    scope: {},
    template:
      '<nav class="vertical hash-magnet floating" stick-to-top>' +
    ...
    link: function (scope, element, attributes) {
      ...
      setTimeout(function () {
        ...
        element.height(
          $(window).height() - parseInt(element.css("margin-bottom")) - 100
        );
        scope.stepsLength = parseInt(
          element.find(".content")[0].scrollHeight / element.height()
        );
      }, 800);
  ...
  return {
      ...
      function ($scope, $filter, $timeout) {
        ...
        $scope.toggleItem = function (item) {
          ...
          if (idx >= 0) {
            ...
            !$scope.maxSelected ||
            $scope.filteredModel.length < $scope.maxSelected
          ) {
        ...
        $scope.fairInclusion = function (anyString, challenger) {
          return (
            lang
              .removeAccents(anyString.toLowerCase())
              .indexOf(lang.removeAccents(challenger.toLowerCase())) >= 0
          );
        };

        $scope.filteringFun = function (item) {
          var precondition = $scope.filterModel()
            ? $scope.filterModel()(item)
            : true;
          if ($scope.searchOn && item instanceof Object)
            return (
              precondition &&
              $scope.fairInclusion(item[$scope.searchOn], $scope.search.input)
            );
          return (
            precondition && $scope.fairInclusion(item, $scope.search.input)
          );
        };
    ...
    link: function (scope, element, attributes) {
      ...
      scope.setComboPosition = function () {
        ...
        element
          .find(".multi-combo-root-panel")
          .css("top", element.find(".multi-combo-root-button").outerHeight());
      };
      scope.setComboPosition();
    },
  };
          ...
          function setBox(apply?) {
            if (ui.breakpoints.checkMaxWidth("tablette")) {
              if (!$(".microbox-wrapper").length) {
                ...
                element.after(
                  '<button class="microbox">' + microtitle + "</button>"
                );
                ...
                $("button.microbox-close, .microbox-content i.close-2x").on(
                  "click",
                  function () {
                    if (!$(".microbox-wrapper").hasClass("zero")) {
...
module.directive("checkTool", function () {
  return {
    ...
    link: function (scope, element, attributes) {
      ...
      $("body").on("click", function (e) {
        if (
          $(e.target).parents(".check-tool, .toggle, .lightbox").length === 0 &&
          !$(e.target).is(".lightbox") &&
          e.target.nodeName !== "CHECK-TOOL" &&
          $("body").find(e.target).length !== 0
        ) {
          scope.ngModel = false;
          element.removeClass("selected");
          checkToolDelay(scope);
...
module.controller("Account", [
  ...
  function ($scope) {
    ...
    $scope.refreshAvatar = function () {
      http()
        .get("/userbook/api/person", {}, { requestName: "refreshAvatar" })
        .done(function (result) {
          $scope.avatar = result.result["0"].photo;
          if (
            !$scope.avatar ||
            $scope.avatar === "no-avatar.jpg" ||
            $scope.avatar === "no-avatar.svg"
          ) {
    ...
    http()
      .get("/directory/userbook/" + model.me.userId)
      .done(function (data) {
...
module.controller("Admin", [
  ...
  function ($scope) {
    ...
    $scope.filterUrls = function (url) {
      return !url.allowed || !(url.allowed instanceof Array)
        ? true
        : _.find(model.me.functions, function (f) {
...
$(document).ready(function () {
  setTimeout(function () {
    ...
    bootstrap(function () {
      ...
      http()
        .get(skin.basePath + "js/directives.js")
        .done((d) => {
          eval(d);
...
module.controller("SearchPortal", [
  "$scope",
  function ($scope) {
...
module.service("CounterService", [
  (function () {
    // CounterService should be defined in its own source file, instead of locally.
