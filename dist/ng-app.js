...
var module = angular.module('app', ['ngSanitize', 'ngRoute'], function ($interpolateProvider) {
    ...
    return idiom_1.idiom;
})
    .factory('model', function ($timeout) {
    var fa = model_1.Collection.prototype.trigger;
    model_1.Collection.prototype.trigger = function (event) {
        $timeout(function () {
            fa.call(this, event);
        }.bind(this), 10);
    };
    var fn = model_1.Model.prototype.trigger;
    model_1.Model.prototype.trigger = function (event, eventData) {
        $timeout(function () {
        ...
        link: function (scope, element, attributes) {
            ...
            scope.$watch(function () { return scope.display.chooseLink; }, function (newVal) {
                if (newVal) {
                    ...
                    var bookmarks = contextEditor.getSelection().createBookmarks(), range = contextEditor.getSelection().getRanges()[0], fragment = range.clone().cloneContents();
            ...
            http_1.http().get('/resources-applications').done(function (apps) {
                scope.linker.apps = [];
                var n = model_1.model.me.apps.length;
                model_1.model.me.apps.forEach(function (app) {
                    ...
                    if (prefix) {
                        ...
                        behaviours_1.Behaviours.loadBehaviours(app.prefix, function (behaviours) {
                            n--;
            ...
            scope.linker.saveLink = function () {
                ...
                var appendText = "", childList = fragment.getChildren(), childCount = childList.count();
    ...
    return {
        ...
        controller: function ($scope, $timeout) {
            var refreshCalendar = function () {
                model_1.model.calendar.clearScheduleItems();
                $scope.items = _.where(_.map($scope.items, function (item) {
                    item.beginning = item.startMoment;
                    item.end = item.endMoment;
                    return item;
                }), { is_periodic: false });
                model_1.model.calendar.addScheduleItems($scope.items);
                $scope.calendar = model_1.model.calendar;
                $scope.moment = moment;
                ...
                $scope.createItem = function (day, timeslot) {
                    $scope.newItem = {};
                    var year = model_1.model.calendar.year;
                    if (day.index < model_1.model.calendar.firstDay.dayOfYear()) {
                        year++;
                    }
                    ...
                    model_1.model.calendar.newItem = $scope.newItem;
                    $scope.onCreateOpen();
                };
                ...
                $scope.updateCalendarWeek = function () {
                    ...
                    if (moment(model_1.model.calendar.dayForWeek).week() === 1 && moment(model_1.model.calendar.dayForWeek).dayOfYear() > 7) {
                        model_1.model.calendar = new calendar_1.calendar.Calendar({ week: moment(model_1.model.calendar.dayForWeek).week(), year: moment(model_1.model.calendar.dayForWeek).year() + 1 });
                    }
                    else if (moment(model_1.model.calendar.dayForWeek).week() === 53 && moment(model_1.model.calendar.dayForWeek).dayOfYear() < 7) {
                        model_1.model.calendar = new calendar_1.calendar.Calendar({ week: moment(model_1.model.calendar.dayForWeek).week(), year: moment(model_1.model.calendar.dayForWeek).year() - 1 });
                    }
                    else {
                        model_1.model.calendar = new calendar_1.calendar.Calendar({ week: moment(model_1.model.calendar.dayForWeek).week(), year: moment(model_1.model.calendar.dayForWeek).year() });
                    }
                    model_1.model.trigger('calendar.date-change');
                    refreshCalendar();
                };
    ...
    return {
        ...
        link: function (scope, element, attributes) {
            ...
            var getTimeFromBoundaries = function () {
                ...
                startTime.year(model_1.model.calendar.year);
                endTime.year(model_1.model.calendar.year);
                ...
                days.each(function (index, item) {
                    ...
                    if (itemLeft < center && itemLeft + dayWidth > center) {
                        var day = index + 1;
                        var week = model_1.model.calendar.week;
                        endTime.week(week);
                        startTime.week(week);
                        if (day === 7) {
                            day = 0;
            ...
            scheduleItemEl.on('stopResize', function () {
                ...
                if (typeof scope.item.calendarUpdate === 'function') {
                    ...
                    model_1.model.calendar.clearScheduleItems();
                    model_1.model.calendar.addScheduleItems(scope.$parent.items);
        ...
        link: function (scope, element) {
            scope.$watch('bindHtml', function (newVal) {
                ...
                if ($(htmlContent).find('.math-tex').length > 0) {
                    if (!window.MathJax) {
                        http_1.http().get('/infra/public/mathjax/MathJax.js?config=TeX-AMS-MML_HTMLorMML').done(function (d) {
                            eval(d);
...
module.directive('portal', function ($compile) {
    return {
        restrict: 'E',
        transclude: true,
        templateUrl: skin_1.skin.portalTemplate,
        compile: function (element, attributes, transclude) {
            element.find('[logout]').attr('href', '/auth/logout?callback=' + skin_1.skin.logoutCallback);
            ui_1.ui.setStyle(skin_1.skin.theme);
...
module.directive('adminPortal', function ($compile) {
    skin_1.skin.skin = 'admin';
    skin_1.skin.theme = '/public/admin/default/';
    return {
        restrict: 'E',
        transclude: true,
        templateUrl: '/public/admin/portal.html',
        compile: function (element, attributes, transclude) {
            $('[logout]').attr('href', '/auth/logout?callback=' + skin_1.skin.logoutCallback);
    ...
    return {
        ...
        link: function (scope, element, attributes) {
            ...
            if (attributes.protected !== undefined) {
                recorder_1.recorder.protected = true;
            }
            recorder_1.recorder.state(function () {
            ...
            scope.saveRecord = function () {
                recorder_1.recorder.save(function (doc) {
                    scope.ngModel = doc;
                    scope.onUpload();
                    scope.$apply();
                });
    ...
    return {
        ...
        link: function (scope, element, attributes) {
            attributes.$observe('workflow', function () {
                ...
                var right = model_1.model.me.workflow;
...
module.directive('userRole', function ($compile) {
    return {
        ...
        link: function ($scope, $element, $attributes) {
            ...
            if (!model_1.model.me.functions[auth]) {
                $element.hide();
            }
            else {
                $element.show();
        ...
        link: function ($scope, $element, $attributes) {
            ...
            $scope.$watch('resource', function (newVal) {
                var hide = ($scope.resource instanceof Array && _.find($scope.resource, function (resource) { return !resource.myRights || resource.myRights[$attributes.name] === undefined; }) !== undefined) ||
                    ($scope.resource instanceof model_1.Model && (!$scope.resource.myRights || $scope.resource.myRights[$attributes.name] === undefined));
                if (hide) {
                    content.hide();
                }
                else {
        ...
        link: function (scope, element, attributes) {
            ...
            var switchHide = function () {
                ...
                var hide = name && (resource instanceof Array && _.find(resource, function (resource) { return !resource.myRights || resource.myRights[name] === undefined; }) !== undefined) ||
                    (resource instanceof model_1.Model && (!resource.myRights || resource.myRights[name] === undefined));
                if (hide) {
                    content.remove();
                    element.hide();
                }
    ...
    return {
        ...
        link: function ($scope, $element, $attributes) {
            function chooseTemplate() {
                if (ui_1.ui.breakpoints.checkMaxWidth("fatMobile")) {
                    $scope.mobile = true;
...
module.directive('widgets', function ($compile) {
    return {
        ...
        link: function (scope, element, attributes) {
            ...
            model_1.model.widgets.on('change', function () {
...
module.directive('attachments', function ($parse) {
    return {
        ...
        link: function (scope, element, attributes) {
            ...
            http_1.http().get('/resources-applications').done(function (apps) {
                scope.attachments.apps = _.filter(model_1.model.me.apps, function (app) {
        ...
        link: function (scope, element, attributes) {
            ...
            function addRemoveEvents() {
                if (ui_1.ui.breakpoints.checkMaxWidth(maxWidth)) {
                    ...
                    ui_1.ui.extendElement.touchEvents(body, { allowDefault: true });
...
module.directive('microbox', function ($compile) {
    return {
        ...
        compile: function (element, attributes, transclude) {
            ...
            return function (scope, element, attributes) {
                var title = idiom_1.idiom.translate(attributes.title);
        ...
        model_1.bootstrap(function () {
            ...
            function start() {
                idiom_1.idiom.addBundle('/i18n', function () {
                    idiom_1.idiom.addBundle('/' + appPrefix + '/i18n', function () {
...
function Account($scope) {
    ...
    $scope.me = model_1.model.me;
    $scope.rand = Math.random();
    $scope.skin = skin_1.skin;
    ...
    $scope.refreshAvatar = function () {
        http_1.http().get('/userbook/api/person', {}, { requestName: "refreshAvatar" }).done(function (result) {
            ...
            model_1.model.me.profiles = result.result['0'].type;
            $scope.$apply();
        });
    };
    ...
    http_1.http().get('/directory/userbook/' + model_1.model.me.userId).done(function (data) {
        model_1.model.me.userbook = data;
        $scope.$apply('me');
    });
    skin_1.skin.listThemes(function (themes) {
        $scope.themes = themes;
        $scope.$apply('themes');
    });
...
function Share($rootScope, $scope, ui, _, lang) {
    ...
    if ($scope.resources instanceof model_1.Model) {
    ...
    $scope.findUserOrGroup = function () {
        ...
        $scope.found = _.union(_.filter($scope.sharingModel.groups.visibles, function (group) {
...
function Admin($scope) {
    ...
    $scope.filterUrls = function (url) {
        return !url.allowed || !(url.allowed instanceof Array) ? true : _.find(model_1.model.me.functions, function (f) { return _.contains(url.allowed, f.code); });
    };
...
Widget.prototype.switchHide = function () {
    ...
    this.hide = !this.hide;
    this.trigger('change');
    model_1.model.widgets.trigger('change');
    model_1.model.widgets.savePreferences();
};
function WidgetModel() {
    model_1.model.makeModels([Widget]);
    model_1.model.collection(Widget, {
        preferences: {},
        ...
        sync: function () {
            var that = this;
            var data = model_1.model.me.widgets;
            http_1.http().get('/userbook/preference/widgets').done(function (pref) {
                ...
                that.load(data, function (widget) {
                    if (widget.i18n) {
                        idiom_1.idiom.addTranslations(widget.i18n);
                    }
                    else {
                    }
                });
        ...
        apply: function () {
            model_1.model.trigger('widgets.change');
        }
    });
}
...
var workspace = {
    ...
    Folder: function (data) {
        ...
        this.collection(workspace.Folder, {
            sync: function () {
                this.load(_.filter(model_1.model.mediaLibrary.myDocuments.folders.list, function (folder) {
    ...
    MyDocuments: function () {
        this.collection(workspace.Folder, {
            sync: function () {
                if (model_1.model.me.workflow.workspace.create) {
    ...
    SharedDocuments: function () {
        this.collection(workspace.Document, {
            sync: function () {
                if (model_1.model.me.workflow.workspace.list) {
...
workspace.Document.prototype.upload = function (file, requestName, callback, visibility) {
    ...
    http_1.http().postFile('/workspace/document?' + visibility + '=true&application=media-library&quality=' + workspace.quality + '&' + workspace.thumbnails, formData, { requestName: requestName }).done(function (data) {
...
function MediaLibrary($scope) {
    if (!model_1.model.mediaLibrary) {
        model_1.model.makeModels(workspace);
        model_1.model.mediaLibrary = new model_1.Model();
        model_1.model.mediaLibrary.myDocuments = new workspace.MyDocuments();
        model_1.model.mediaLibrary.sharedDocuments = new workspace.SharedDocuments();
        model_1.model.mediaLibrary.appDocuments = new workspace.AppDocuments();
        model_1.model.mediaLibrary.publicDocuments = new workspace.PublicDocuments();
    }
    $scope.myDocuments = model_1.model.mediaLibrary.myDocuments;
    $scope.display = {
        show: 'browse',
        search: ''
    };
    ...
    $scope.listFrom = function (listName) {
        ...
        model_1.model.mediaLibrary[$scope.display.listFrom].sync();
    };
    ...
    $scope.$watch('visibility', function (newVal) {
        ...
        else if (model_1.model.me.workflow.workspace.list) {
            $scope.display.listFrom = 'sharedDocuments';
        }
        model_1.model.mediaLibrary.on('myDocuments.sync, sharedDocuments.sync, appDocuments.sync, publicDocuments.sync', function () {
            $scope.documents = filteredDocuments(model_1.model.mediaLibrary[$scope.display.listFrom]);
            if (model_1.model.mediaLibrary[$scope.display.listFrom].folders) {
                $scope.folders = model_1.model.mediaLibrary[$scope.display.listFrom].folders.filter(function (folder) {
            ...
            $scope.folder = model_1.model.mediaLibrary[$scope.display.listFrom];
        ...
        $scope.$watch('fileFormat', function (newVal) {
            ...
            if (model_1.model.mediaLibrary[$scope.display.listFrom].documents.length() === 0) {
                model_1.model.mediaLibrary[$scope.display.listFrom].sync();
            }
            else {
                model_1.model.mediaLibrary[$scope.display.listFrom].trigger('sync');
            }
        });
    });
    ...
    $scope.insertRecord = function () {
        model_1.model.mediaLibrary.appDocuments.documents.sync();
        $scope.display.show = 'browse';
        $scope.listFrom('appDocuments');
    };
    $scope.selectDocument = function (document) {
        if (($scope.folder === model_1.model.mediaLibrary.appDocuments && $scope.visibility === 'protected') ||
            ($scope.folder === model_1.model.mediaLibrary.publicDocuments && $scope.visibility === 'public')) {
            if ($scope.multiple) {
